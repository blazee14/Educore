...
import { docenteNavGroups, docenteHomeItem } from '../config/docenteNav';
...
export function SidebarDocente({ open, onClose }: Props) {
      ...
      >
        <div className="flex items-center gap-3 border-b border-white/10 px-5 py-5">
          ...
          <div>
            ...
            <p className="text-xs text-blue-100/70">Portal del Docente</p>
          </div>
        </div>

        <nav className="flex-1 overflow-y-auto px-3 py-4">
          <NavLink
            to={docenteHomeItem.to}
            end
            className={({ isActive }) => `${linkBase} ${isActive ? linkActive : linkInactive} mb-4`}
            onClick={onClose}
          >
            <docenteHomeItem.icon className="h-[18px] w-[18px]" />
            {docenteHomeItem.label}
          </NavLink>

          {docenteNavGroups.map((grupo) => (
        ...
        <div className="border-t border-white/10 p-4">
          <div className="mb-3 flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-white">Docente</p>
